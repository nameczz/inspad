...
import i18n from 'lang/research/cn-term'
export default {
  components: {
    JsonSchema,
  },
  ...
  methods: {
    async translate() {
      this.loading = true
      try {
        let res = await apiResearch.getCnTerm({
          topn: 10,
          text: this.inputText,
        })
        this.json = res
        if(res['error_code'] === 0) {
          this.outputText = res.data.tfidf.map(item => item.keyword).join(',')
        }
      } finally {
        this.loading = false
      }
